...
class ResidualNet(ResNet):
    ...
    def forward(self, x):
        ...
        # print("max_pool.size -- ", x.size()) -- [1, 2048, 3, 3]
        ...
        # print("avg_pool.size -- ", x.size()) -- [1, 2048, 3, 3]
        ...
        # (Pdb) pp output['max'].size() -- [1, 2048]
        # (Pdb) pp output['avg'].size() -- [1, 2048]
        # (Pdb) pp output['fc'].size() -- [1, 1000]
    ...
    def make_samples(self, db, verbose=True):
        ...
        try:
            ...
            for d in data.itertuples():
                ...
                # img.shape -- (3, 84, 84)
                # (Pdb) img.mean(), img.max() -- (0.4726316452506929, 1.0)

                # pp means -- array([0.40760392, 0.45795686, 0.48501961])
                ...
                try:
                    if use_gpu:
                        inputs = torch.from_numpy(img).cuda().float()
                    else:
                    ...
                    pass
            cPickle.dump(
                samples, open(os.path.join(cache_dir, sample_cache), "wb", True)
            )
